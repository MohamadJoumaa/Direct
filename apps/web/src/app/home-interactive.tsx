...
function sendHref(signedIn: boolean) {
  return signedIn ? "/app/client/new" : "/register";
}

const navClassName =
  "touch-target inline-flex items-center whitespace-nowrap rounded-full px-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground";
...
export function HomeHeader() {
  ...
  const marketingNav = [
    { id: "send", href: sendHref(Boolean(user)), label: dict.nav.send },
      ...
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4">
        ...
        <nav className="hidden min-w-0 flex-1 items-center justify-center gap-0.5 lg:flex">
          {marketingNav.map((item) => (
            <Link key={item.id} href={item.href} className={navClassName}>
              {item.label}
            </Link>
          ))}
        </nav>
...
export function HomeMarketing() {
  const { user } = useAuth();
  const { dict } = useI18n();
  const send = sendHref(Boolean(user));
